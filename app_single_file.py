...
<style>
  ...
  /* ---- compact density: smaller base type, tighter spacing ---- */
  html {{ font-size: 14px; }}
  .block-container {{ padding-top: 2.6rem; padding-bottom: 2rem; max-width: 100%; }}
  ...
  [data-testid="stFileUploaderDropzoneInstructions"] {{ font-size: 0.78rem; }}
  [data-testid="stFileUploaderDropzoneInstructions"] span {{ font-size: 0.9rem; }}
  /* keep Material icons from spilling as raw ligature text if the font lags */
  [data-testid="stExpander"] summary svg {{ flex: 0 0 auto; }}
  .stExpander details summary p {{ font-size: 0.9rem; }}

  /* Always-visible "working" indicator: Streamlit's status widget (top-right)
     shows a spinner + "Running" while anything computes, so a slow parse never
     looks frozen. */
  [data-testid="stStatusWidget"] {{ visibility: visible !important; opacity: 1 !important; }}
  [data-testid="stStatusWidget"] {{ background: {NAVY}; color: #fff; border-radius: 6px;
      padding: 2px 8px; box-shadow: 0 2px 8px rgba(0,40,86,.25); }}
  [data-testid="stStatusWidget"] label, [data-testid="stStatusWidget"] * {{ color:#fff !important; }}
</style>
"""
...
STRINGS: dict[str, dict[str, str]] = {
    # ---------------------------------------------------------------- shell --
    "app_title":       {"es": "Análisis de Ventas", "en": "Sales Analytics"},
    ...
    "upload_prev":     {"es": "Mes anterior (YTD del cierre pasado)",
                        "en": "Previous month (last close's YTD)"},
    "upload_prev_help": {"es": "Opcional. El mismo export YTD descargado el mes pasado. Habilita la pestaña de evolución mensual.",
                         "en": "Optional. The same YTD export downloaded last month. Enables the monthly evolution tab."},
...
# --------------------------------------------------------------------------- #
_PT_FR = {
    "app_title": {"pt": "Análise de Vendas", "fr": "Analyse des Ventes"},
...
def _names(frame: pd.DataFrame, col: str, n: int = 5) -> str:
...
def opportunities(cmp_cust: pd.DataFrame, cmp_prod: pd.DataFrame,
    ...
    if not grow.empty:
        top = grow.sort_values("sales_delta", ascending=False).head(5)
    ...
    if not cmp_prod.empty and "margin_pct_cur" in cmp_prod.columns:
        ...
        if not rich.empty:
            small = rich.sort_values("sales_cur").head(5)
            detail = ", ".join(
...
def actions(cmp_cust: pd.DataFrame, forecast: pd.DataFrame | None,
    ...
    if not lost.empty:
        out.append(t("act_churn", names=_names(lost, label_col),
                     total=M(lost["sales_base"].head(5).sum())))
...
def build_sidebar(ytd, fy) -> Context:
    ...
    return Context(
        ...
        active_metrics=active, lang=st.session_state.get("lang", "en"),
...
def render(ctx) -> None:
    ...
    st.markdown(NOTES_ES if st.session_state.get("lang", "en") == "es" else NOTES_EN)
